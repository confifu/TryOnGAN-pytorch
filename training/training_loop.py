...
def training_loop(
    ...
    if rank == 0:
        ...
        images = torch.cat([torch.sum(G_ema(z=z, c=c, pose=pose, ret_pose = True, noise_mode='const')[0], dim=1,keepdim=True).cpu() for z, c, pose in zip(grid_z, grid_c, grid_pose)]).numpy()
        save_image_grid(images, os.path.join(run_dir, 'fakes_pose_init.png'), drange=[0,1], grid_size=grid_size)

        i=4
        bina = torch.cat([G_ema(z=z, c=c, pose=pose, ret_pose = True, noise_mode='const')[1][i].cpu() for z, c, pose in zip(grid_z, grid_c, grid_pose)]).numpy()
        save_image_grid(bina, os.path.join(run_dir, 'fakes_bin_' + str(i) + 'init.png'), drange=[-1,1], grid_size=grid_size)
        col = torch.cat([G_ema(z=z, c=c, pose=pose, ret_pose = True, noise_mode='const')[2][i].cpu() for z, c, pose in zip(grid_z, grid_c, grid_pose)]).numpy()
        save_image_grid(col, os.path.join(run_dir, 'fakes_col' + str(i) + 'init.png'), drange=[-1,1], grid_size=grid_size)
        images = torch.cat([G_ema(z=z, c=c, pose=pose, ret_pose = True, noise_mode='const')[3].cpu() for z, c, pose in zip(grid_z, grid_c, grid_pose)]).numpy()
    ...
    while True:
        ...
        for phase, phase_gen_z, phase_gen_c in zip(phases, all_gen_z, all_gen_c):
            ...
            for round_idx, (real_img, real_pmap, pose, real_c, gen_z, gen_c) in enumerate(zip(phase_real_img, phase_real_pmap, phase_pose, phase_real_c, phase_gen_z, phase_gen_c)):
                ...
                gain = phase.interval
                loss.accumulate_gradients(phase=phase.name, real_img=real_img, real_pmap=real_pmap, real_pose = pose, real_c=real_c, gen_z=gen_z, gen_c=gen_c, sync=sync, gain=gain, traintill=traintill)
