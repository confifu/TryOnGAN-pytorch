...
class SynthesisLayer(torch.nn.Module):
    ...
    def forward(self, x, w, styles=None, noise_mode='random', fused_modconv=True, gain=1):
        '''
        output will have resolution self.resolution
        '''
        assert noise_mode in ['random', 'const', 'none']
        if self.up > 1:
            in_resolution = self.resolution // self.up
        elif self.down > 1:
            in_resolution = self.resolution * self.down
        else :
...
class SynthesisBlock(torch.nn.Module):
    def __init__(self,
        ...
        **layer_kwargs,                     # Arguments for SynthesisLayer.
        ...
        if in_channels != 0:
            self.conv0 = SynthesisLayer(in_channels, out_channels, w_dim=w_dim, resolution=resolution, up=self.up, down=self.down,
        ...
        if in_channels != 0 and architecture == 'resnet':
            self.skip = Conv2dLayer(in_channels, out_channels, kernel_size=1, bias=False, up=self.up, down = self.down,
    ...
    def __init__(self,
        ...
        **block_kwargs,             # Arguments for SynthesisBlock.
        ...
        for i in range(3):
            ...
            in_channel = 7 * channels_dict[reslist[i]] if i > 0 else 17
        ...
        for i in range(2):
            ...
            self.num_ws += block.num_conv + block.num_torgb
            setattr(self, f'finalBlock{i}', block)
    ...
    def forward(self, ws, pose, ret_pose = False, **block_kwargs):
        ...
        for i in range(3):
            for j in range(7):
                ...
                xs[j], bin_regions[j], w_idx = self.get_block_output(block, ws, w_idx, input, bin_regions[j], **block_kwargs)
            input = torch.cat([xs[k] for k in range(7)], dim = 1)
        ...
        for i in range(4):
            ...
            for j in range(7):
                ...
                xs[j], col_regions[j], w_idx = self.get_block_output(block, ws, w_idx, input, col_regions[j], **block_kwargs)
        ...
        img = None
        for i in range(2):
            block = getattr(self, f'finalBlock{i}')
            x, img, w_idx = self.get_block_output(block, ws, w_idx, x, img, **block_kwargs)
        if ret_pose:
    ...
    def __init__(self,
        ...
        for res in self.block_resolutions:
            ...
            block = DiscriminatorBlock(in_channels, tmp_channels, out_channels, resolution=res,
                img_channels = img_channels,first_layer_idx=cur_layer_idx, use_fp16=use_fp16, **block_kwargs, **common_kwargs)
